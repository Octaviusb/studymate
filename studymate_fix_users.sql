-- ===============================
-- STUDYMATE SAAS - ARREGLAR TABLA USERS
-- ===============================
...
ALTER TABLE `users` ADD COLUMN `organization_id` int(11) NOT NULL DEFAULT 1 AFTER `id`;

-- Agregar columna role si no existe
ALTER TABLE `users` ADD COLUMN `role` enum('super_admin','admin','coordinator','secretary','teacher','student','parent') NOT NULL DEFAULT 'student' AFTER `password`;
...
ALTER TABLE `users` ADD COLUMN `created_at` timestamp NOT NULL DEFAULT current_timestamp() AFTER `status`;
